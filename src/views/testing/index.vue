<template>
  <div class="app-container documentation-container">
    <el-button class="tutorial_btn" type="warning" icon="el-icon-guide"  @click.prevent.stop="guide">Turtorial</el-button>
      ...
      <aside style="height: 100%; min-width: 300px;" id="parameter-panel">
        ...
        <el-form ref="postForm" :model="postForm" :rules="rules" class="form-container">
          ...
          <div style="display: flex; justify-content: space-between;" id="uploadFiles">
            ...
            >
              <el-button v-if="!canClick" size="small" type="primary" style="width: 100%;"><i class="el-icon-upload" />
                {{$t('global.trainingFile')}}</el-button>
              <el-button v-else slot="tip" size="small" type="primary" disabled><i class="el-icon-upload" /> {{$t('global.trainingFile')}}</el-button>
              <!-- <div slot="tip" class="el-upload__tip">最多可上传5个，文件大小不超过50MB</div> -->
            </el-upload>
          </div>
          <el-button style="width: 100%;" @click="validation" id="crossvalBtn" type="success" plain>{{$t('global.validation')}}</el-button>
        </el-form>
      </aside>
      <div>
        <div class="training_results">
          <el-card id="crossvalRes"
        ...
        <el-card
          ...
          element-loading-text="Tree Loading..."
        ...
        >
          ...
          >
            <el-card
              style="padding:0;background-color: rgba(70, 70, 70, 0.1); color:#000; min-height: 400px;"
            >
            ...
            <div style="flex:1; padding: 10px; border: 1px dashed #000; position: relative;">
              <h3 style="text-align: center;">{{$t('global.treeModelVis')}} | Tree:{{ currentTreeIndex }}</h3>
              <el-divider />
              <div class="node-info">
...
import { startValidation, getTestTreeData,getValRes } from '@/api/silas'
import * as d3 from 'd3'
import ace from 'ace-builds/src-noconflict/ace'
import 'ace-builds/src-noconflict/mode-javascript' // 例如：JavaScript 模式
import 'ace-builds/src-noconflict/theme-monokai' // 例如：Monokai 主题
...
export default {
  ...
  methods: {
    ...
    validation() {
      const that = this
      startValidation(this.postForm).then(res => {
        const result = res.TrainResult
